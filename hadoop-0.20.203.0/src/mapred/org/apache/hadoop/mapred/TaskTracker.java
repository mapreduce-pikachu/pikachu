...
public class TaskTracker implements MRConstants, TaskUmbilicalProtocol,
 ...
 /* Rohan Variables */
  List<Integer> reducerPID = new ArrayList<Integer>();
  Thread cpuThread;
  ...
  private RunningJob addTaskToJob(JobID jobId, 
    ...
    synchronized (runningJobs) {
      ...
      if (!runningJobs.containsKey(jobId)) {
        ...
        rJob.virtualMappings = new VirtualMappingStatus(jobId);
 ...
 //  cpuThread.run();
  ...
  public synchronized void setCPUMonitoring(){

	  if (++setCPUMonitor == this.maxReduceSlots){
		  //s Starting the thread to monitor the CPU usge //
		  System.out.println("Hello-from-thread-wrapper!");
		  cpuThread =  new Thread(new cpuUsageThread());
  ...
  public class VirtualMappingStatus {
    private JobID jobId;    
    private List<virtualMapper> allVirtualMappers;
    
    public VirtualMappingStatus(JobID jobId) {
      this.jobId = jobId;
      this.allVirtualMappers = new ArrayList<virtualMapper>(5);
    }
    
    public virtualMapper[] getVirtualMapping(int fromId) {
      virtualMapper[] newVirtualMapper = new virtualMapper[0];
      synchronized (allVirtualMappers) {
        if (allVirtualMappers.size() > fromId) {
          List <virtualMapper> eventSublist = 
            allVirtualMappers.subList(fromId, allVirtualMappers.size());
          newVirtualMapper = eventSublist.toArray(newVirtualMapper);
        }
      }
      return newVirtualMapper;
    }
    
    public void addVirtualMapping(virtualMapper newVirtualMapper)
    {
      int ver = newVirtualMapper.getVersion();
      LOG.info("Add virtual mapper version " + ver + " for Job: " + jobId);
      synchronized (allVirtualMappers) {
        if (ver < allVirtualMappers.size()) {
          if (allVirtualMappers.get(ver) != null) {
            LOG.info("Warning: [addVirtualMapping] overwrite the existing version " + ver);
          }
          allVirtualMappers.set(ver, newVirtualMapper);
        }
        else {
          if (ver != allVirtualMappers.size()) {
            LOG.info("Warning: [addVirtualMapping] get version " + ver 
                + ", but expect version " + allVirtualMappers.size());
          }
          allVirtualMappers.add(newVirtualMapper);
          newVirtualMapper.getReducerBins(1);
        }
      }
    }
      ...
      try {
        ...
        if (actions != null){ 
          for(TaskTrackerAction action: actions) {
            ...
            } else if (action instanceof NewMappingAction) {
              LOG.info("Processing NewMappingAction...");
              addToVirtualMappings((NewMappingAction) action);
              LOG.info("Done NewMappingAction...");
            } else {
  ...
  public class cpuUsageThread extends Thread {
	  ...
	  public void changeMonitor (boolean state){
		  monitor = state;
	  }
	  
	  public void run() {
		  System.out.println("Hello-from-thread!");
		  // Start the CPU monitor after 12 sec so the reducer processes are initiated.
		  try {
		  ...
		  Iterator<Integer> reducerPIDIterator = reducerPID.iterator();
		  int pid;
		  
		  String command = new String("/home/gandhir/Hadoop_Setup-socc/hadoop-0.20.203.0/bin/proc-stats/cpu-hadoop/getCPU-reducer.pl");
		 
		  while (reducerPIDIterator.hasNext()){
			  pid = reducerPIDIterator.next();
			  command = command + " " + pid ;
			  System.out.println("The redpid: " + pid);
		  }
		  System.out.println("The command-sent: " + command);
		  Process p;
		  try {
...
/*
  public class cpuUsageThread extends Thread {
	  ...
	  public void run() {
		  ...
		  float cpuUsage = 0;
		  float cpuUsageCumulative = 0;
		  float cpuUsageCumulativePrev = 0;
		  int timeTotal = 0;
		  long timeStart = System.currentTimeMillis();
		  float movingAverage[] = new float[20];
		 
		  
		  try {
			  BufferedWriter out = new BufferedWriter(new FileWriter("/export/gandhir/rohan2.txt"));
			  BufferedWriter outCPU = new BufferedWriter(new FileWriter("/export/gandhir/rohan-cpu.txt"));
			  long timeBegin;
			  while (monitor){

				  timeBegin = System.currentTimeMillis();
				  if (timeBegin - timeStart > (20 * 60 * 1000)){ // Stop after 20 min 0 sec.
					  monitor = false;
				  }
				  out.write( "start "+ "\n");
				  out.flush();
				  reducerPIDIterator = reducerPID.iterator();
				  count = 0;
				  average = 0;
				  while (reducerPIDIterator.hasNext()){
					  pid = reducerPIDIterator.next();
					  command = "ps -eo pcpu,pid | grep " + pid + " | grep -v \"grep\" | awk '{print $1}'";
					  ...
					  try {
						  ...
						  cpuUsageCumulative = Float.parseFloat(cpuUsageString) ;
						  // For cummulative values.
						  average = average * count + cpuUsageCumulative;
						  count++;
						  average = average / count;
						  
						  /*
						  cpuUsage = cpuUsageCumulative * (timeTotal + 1) - (cpuUsageCumulativePrev * timeTotal);
						  timeTotal++;
						  cpuUsageCumulativePrev = cpuUsageCumulative;
						  
						  movingAverage[timeTotal % movingAverage.length] = cpuUsage;
						  float total = 0;
						  for (int i=0;i<movingAverage.length;i++){
							  total += movingAverage[i];
						  }
						  average = total / movingAverage.length;
						  
						  average = average * count + cpuUsage;
						  count++;
						  average = average / count;
						   */
						  /*
						  out.write( "CPU-measure " + System.currentTimeMillis() / 1000 
								  + " " + maximumCPUUsage 
								  + " " + average 
								  + " " + cpuUsageCumulative 
								  + " " + cpuUsage
				...
				//  out.write( "time " + timeEnd + " " + (timeEnd-timeBegin) + "\n");
				//  out.flush();
				  out.write( "CPU-measure " + System.currentTimeMillis() / 1000 
						  + " " + maximumCPUUsage 
						  + " " + average );
				  out.flush();
				  if (maximumCPUUsage < average){
					  maximumCPUUsage = average;
				  }

				  outCPU.write(maximumCPUUsage + " " + average + "\n");
				  outCPU.flush();
...
*/
  ...
  HeartbeatResponse transmitHeartBeat(long now) throws IOException {
	  ...
	  LOG.debug("[DiXie] Get heartbeat response ID:" + heartbeatResponseId);
  ...
  private String String(byte[] buffer, int i, int read, String string) {
...
/**
  ...
  void startNewTask(TaskInProgress tip) throws InterruptedException {
    try {
      ...
      /* Rohan: This point we need to start the CPU monitoring */
      /* We monitor when the reducer starts */
      if (tip.task.isMapOrReduce() && !tip.task.isMapTask()){
    	  setCPUMonitoring();
      }
...
//TODO: are these calls really required
  ...
  throws IOException {
    ...
    if (tasks.get(tip.getTask().getTaskID()) != null) { //is task still present
      LOG.info("Rohan JVM with ID: " + jvmId + " given task: " + 
	       tip.getTask().getTaskID() + "||\t" + jvmManager.getPid(tip.getTaskRunner()));

      try {
    	  BufferedWriter out = new BufferedWriter(new FileWriter("/export/gandhir/rohan.txt", true));
    	  out.write( tip.getTask().getTaskID()+ "\t" + jvmManager.getPid(tip.getTaskRunner()) + "\t" + tip.getTask().isJobCleanupTask() + "\n");
    	  //
    	  if (tip.getTask().getTaskID().toString().contains("_r_")){
    		  reducerPID.add(Integer.parseInt(jvmManager.getPid(tip.getTaskRunner())));
    	  }
		//
    	  out.close();
      } catch (IOException e) {
  ...
  static class RunningJob{
    ...
    FetchStatus f;
    VirtualMappingStatus virtualMappings;
    ...
    VirtualMappingStatus getVirtualMappingStatus() {
  ...
  public static class MapOutputServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, 
      ...
      String vbinIDString = request.getParameter("vbin");

      int reduce = Integer.parseInt(reduceId);
      int vbinID = Integer.parseInt(vbinIDString);
      ...
      IndexRecord info = tracker.indexCache.getIndexInformation(mapId, vbinID,indexFileName, 
								runAsUserName);
	...
	response.setHeader(MAP_OUTPUT_LENGTH,
        ...
        System.out.println("Sending the response to " + reduce + "\t" + vbinID);
  ...
  public void addToVirtualMappings(NewMappingAction action) {
    JobID jobId = action.getJobID();
    synchronized (runningJobs) {
      RunningJob rJob = null;
      if (!runningJobs.containsKey(jobId)) {
      ...
      synchronized (rJob) {
        rJob.virtualMappings.addVirtualMapping(action.getVirtualMapper());
      }
    }
  }

  @Override
  public VirtualMappingUpdate getVirtualMappings(JobID jobId, int fromIndex)
      throws IOException {
    authorizeJVM(jobId);
    virtualMapper []mapEvents = new virtualMapper[0];
    RunningJob rjob;
    synchronized (runningJobs) {
      rjob = runningJobs.get(jobId);          
      if (rjob != null) {
        synchronized (rjob) {
          VirtualMappingStatus vms = rjob.getVirtualMappingStatus();
          if (vms != null) {
            mapEvents = vms.getVirtualMapping(fromIndex);
          }
        }
      }
    }
    return new VirtualMappingUpdate(mapEvents);
  }
