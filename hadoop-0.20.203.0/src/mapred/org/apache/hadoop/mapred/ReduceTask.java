...
class ReduceTask extends Task {
  ...
  void runOldReducer(JobConf job,
    ...
    try {
      ...
      while (values.more()) {
    	  //System.out.println("ReduceTask: reduceInputKeyCounter" + 
    	//		  reduceInputKeyCounter.getCounter());
  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    /**
     * a list of map output locations for successful fetches 
     */
    private List<MapOutputLocation> completeFetches =
      new ArrayList<MapOutputLocation>();
    
    private VirtualMappingAll virtualMappingAll;
    ...
    private class MapOutput {
      ...
      public void setvbinID(int recvVbinID) {
        vbinID = recvVbinID;
      }      
   ...
   //     float percentUsed;
        ...
        //	percentUsed = getPercentUsed();
        	/* Rohan For tracing the wait time */
        	/*
        	System.out.println("percentUsed:" + percentUsed + " " + maxInMemCopyPer + " "
        	+ maxInMemOutputs + " " + numClosed + " " + 
        			numPendingRequests + " " + numRequiredMapOutputs);
        	*/
        	System.out.println("Sort-Wait: " + System.currentTimeMillis() / 1000);
          ...
          System.out.println("percentUsed:" + percentUsed + " " 
              	+ maxInMemOutputs + " " + numClosed + " " + 
              			numPendingRequests + " " + numRequiredMapOutputs);
              			*/
         // System.out.println("numCopiers:" + numCopiers + " " + MAX_STALLED_SHUFFLE_THREADS_FRACTION + " "
         //       	);
          System.out.println("Sort-Free: " + System.currentTimeMillis() / 1000);	
          done = closed;
        }
        return done;
      }
  ...
  public List<Integer> getReducerBins(int reduce){
    ...
    */
    ...
    private class MapOutputCopier extends Thread {
       ...
       * IMPORTANT: it should be called after the "scheduledCopies" being cleared
       */
        ...
        while (true) {        
          try {
            ...
            synchronized (scheduledCopies) {
              ...
              //updateneeded = isupdateneeded(loc);
              /*System.out.println("rohanloc: Got location " + loc.getTaskId().toString()
            		  + " updateneeded " + Boolean.toString(updateneeded) );
              */
            ...
            if (updateneeded == true){
            	...
            	try {
            		...
            		/* Rohan added on Sept 26, this might be increasing the success count
            		 * unnecessarily killing the progress score.
            		 */
      ...
      private boolean isupdateneeded(MapOutputLocation loc){
    	  TaskAttemptID reduceId = reduceTask.getTaskID();
         
    	  List<Integer> vBins = virtualMappingAll.getReducerBins(reduceId
    			  .getTaskID().getId(), loc.getVersion());
    	  int useVersion = virtualMappingAll.getVersion(); 
    	  System.out.println("rohanvbins: " + System.currentTimeMillis()/1000 + "Mapper " + loc.getTaskId().toString() + 
        			"\tVersion= " + loc.getVersion() + 
        			"\tReduceTask vbins" + Arrays.toString(vBins.toArray()));
    	  
    	  if (vBins.isEmpty() == true){
          	return false;
          }
    	  return true;
      ...
      InterruptedException {
    	  ...
    	  TaskAttemptID reduceId = reduceTask.getTaskID();
    	  // Path filename = new
    	  // Path(String.format(MapOutputFile.REDUCE_INPUT_FILE_FORMAT_STRING,
    	  ...
    	  // Copy the map output
    	  //        virtualMapper myVirtualBinClass = new virtualMapper();
    	  //        myVirtualBinClass.virtualMapper();
    	  //        List<Integer> virtualBins = myVirtualBinClass.getReducerBins(reduceId
    	  //            .getTaskID().getId());
    	  List<Integer> virtualBins = virtualMappingAll.getReducerBins(reduceId
    			  .getTaskID().getId(), loc.getVersion());
    	  int useVersion = virtualMappingAll.getVersion(); 
    	  System.out.println("rohanvbins: " + System.currentTimeMillis()/1000 + 
    			  " Mapper " + loc.getTaskId().toString() + 
      			" Version= " + loc.getVersion() + 
      			" ReduceTask vbins" + Arrays.toString(virtualBins.toArray()));
    	  
    /*	  System.out.println("rohanvbins: Mapper " + loc.getTaskId().toString() + 
      			"\tVersion= " + loc.getVersion() + 
      			"\tReduceTask vbins" + Arrays.toString(virtualBins.toArray()));
     */
    	  Iterator<Integer> itr = virtualBins.iterator();
    	  long totalBytes = 0;
    	  int vbinId = 0;
    	  /* Rohan change september 27 */
    	  
    	  if (virtualBins.isEmpty() == true){
    		  noteCopiedMapOutput(loc, useVersion); // The counter to fetchoutputs needs to be incremented
    		  return 0;
    	  }
    	  else{ 	  
    		  while (itr.hasNext()) {
    			  ...
    			  filename = new Path(String.format(
    					  MapOutputFile.REDUCE_INPUT_FILE_FORMAT_STRING,
    					  TaskTracker.OUTPUT, loc.getTaskId().getId())
    					  + Integer.toString(vbinId));
    			  Path tmpMapOutput1 = new Path(filename + "-" + id);
    			  // System.out.println ("Fetching bin " + vbinId);

    			  URL mytaskOutput = new URL(loc.getOutputLocation().toString()
    					  + "&vbin=" + Integer.toString(vbinId));

    			  MapOutputLocation myloc = new MapOutputLocation(loc
    					  .getTaskAttemptId(), loc.ttHost, mytaskOutput);

    			  MapOutput mapOutput = getMapOutput(myloc, tmpMapOutput1, reduceId
    					  .getTaskID().getId());

    			  mapOutput.setvbinID(virtualBins.get(0));
    			  ...
    			  totalBytes += bytes;
    			  //     System.out.println("Got map output " + bytes);

    			  // lock the ReduceTask while we do the rename
    			  synchronized (ReduceTask.this) {
    				  /*
    				   * if (copiedMapOutputs.contains(loc.getTaskId())) {
    				   * mapOutput.discard(); System.out.println("Reducer: Closing 1");
    				   * return CopyResult.OBSOLETE; }
    				   */
    				  // System.out.println("Reducer: Stage 1");
    				  ...
    				  if (bytes == 0) {
    					  ...
    					  noteCopiedMapOutput(loc, useVersion);

    					  return bytes;
    				  }

    				  // System.out.println("Reducer: Stage 2");

    				  // Process map-output
    				  if (mapOutput.inMemory) {
    					  // Save it in the synchronized list of map-outputs
    					  //   System.out.println("MapOutput: " + mapOutput.toString());
    					  mapOutputsFilesInMemory.add(mapOutput);
    					  //         System.out.println("Saving in the local memory");
    				  } else {
    					  ...
    					  System.out.println("TmpFilename is: " + tmpMapOutput.toString());
    					  System.out.println("Tmpparent: "
    							  + tmpMapOutput.getParent().toString());

    					  filename = new Path(tmpMapOutput.getParent(), filename.getName());
    					  System.out.println("Filename2 is: " + filename.toString());
       ...
       */
      private void noteCopiedMapOutput(MapOutputLocation loc, int ver) {
        copiedMapOutputs.add(loc.getTaskId());
        
   //     System.out.println("rohanvbins: ReduceTask loc: " + loc.taskId.toString() + " setting the version to " + ver);
        loc.setVersion(ver);
        synchronized (completeFetches) {
        	/*Rohan added sept 27: To remove the duplicates.*/
        	if (!completeFetches.contains(loc))
        		completeFetches.add(loc);
        }
        ramManager.setNumCopiedMapOutputs(numMaps - copiedMapOutputs.size());
      //  System.out.println("ReduceTask: MapOutputSize= " + copiedMapOutputs.size());
      }
...
//	System.out.println("Sending reducer request for reduce: " + reduce);        
...
//	System.out.println("Sending reducer request for reduce: " + reduce + "\t" + forReduce);        
  ...
  //  System.out.println("The header: " + msgToEncode);
//	System.out.println("The URL: " + connection.getURL().toString());

	

        String encHash = SecureShuffleUtils.hashFromString(msgToEncode, 
            jobTokenSecret);
	//	System.out.println("The encHash: " + encHash);
      ...
      throws IOException, InterruptedException {
        ...
        //System.out.println("ReduceTask: Length(Map/Compressed) = " + mapOutputLength + compressedLength);
    ...
    /**
     * A class that maintains all the virtual mapping in one reduce task.
     * It can compute the new virtual bins it needs to fetch based on
     * fetched virtual bins.<br>
     * NOTE: It assumes switchToLatest() and getReducerBins(), getVersion()
     * <b>WILL NOT</b> execute at the same time, there is <b>NO</b> lock 
     * on "currentVersion".  
     */
    public class VirtualMappingAll {

    	List<virtualMapper> virtualMappers;
    	/**
    	 * currentVersion indicates the version currently in use, maybe not the latest
    	 * The method switchToLatest() will change currentVersion to maxVersion 
    	 */
    	volatile int currentVersion;
    	/**
    	 * maxVersion is the max version number it has. maxVersion >= currentVersion 
    	 */
    	volatile int maxVersion;
    	boolean reset;

    	public VirtualMappingAll(JobConf conf) {
    		currentVersion = 0;
    		maxVersion = 0;
    		virtualMappers = new ArrayList<virtualMapper>(5);
    		virtualMapper defaultVM = new virtualMapper();
    		defaultVM.setDefault(conf);
    		virtualMappers.add(defaultVM);
    		setStartTime();
    	}
    	...
    	private List<Integer> getList(int reduce, int index){
    		return virtualMappers.get(index).getReducerBins(reduce);  
    	}
      
      
//      public boolean setReducerBins(int[] currentBins){
//        virtualMappers[currentVersion] = new virtualMapper();
//        if (virtualMappers[currentVersion].set(currentBins, (currentVersion + 1)) == true){
//          currentVersion++;
//          return true;
//        }
//        return false;
...
//      }
    	...
    	/**
    	 * Return the current version in use to compute bins.
    	 */
    	public int getVersion() {
    	...
    	/**
    	 * Use the latest version (maxVersion) to compute the virtual bins.<br>
    	 * NOTE: It assumes this <b>WILL NOT</b> execute at the same time with
    	 * getReducerBins(), getVersion(), there is <b>NO</b> lock on
    	 * "currentVersion".
    	 */
    	...
    	/**
    	 * Check if it receives newer versions of mappings, while the current
    	 * one in use needs to update.  
    	 * @return true: if need an update
    	 */
    	...
    	/**
    	 * Add new virtual mappers <br>
    	 * NOTE: Users needs to lock this object before calling this method.
    	 * @param newVM An array of new virtualMappers to add.
    	 */
    	public void add(virtualMapper [] newVM) {
    		for (int i = 0; i < newVM.length; i++) {
    			maxVersion++;
    			virtualMappers.add(newVM[i]);
    			if (newVM[i].getVersion() != maxVersion) {
    				LOG.info("Warning: Version mismatch when adding new virtual mapper " +
    						"to VirtualMappingAll, expect " + maxVersion + " while receive " + 
    						newVM[i].getVersion());
    			}
    		}        
    	}
    	...
    	/**
    	 * Compute the virtual bins it needs to fetch, given it has fetched all the 
    	 * virtual bins specified in version <i>version</i>.
    	 * @param reduce Reduce ID
    	 * @param version The current fetched version. -1 means it hasn't been fetched
    	 * before.
    	 * @return
    	 */
    	public List<Integer> getReducerBins(int reduce, int version) {
    		List<Integer> returnBins;
    		if (version == -1) {
    //			System.out.println("rohanvbins: getReducerBins version = -1");
    			returnBins = getList(reduce, currentVersion);
    		} else {
    //			System.out.println("rohanvbins: getReducerBins version = " + version);
    			
    			// May need optimization to get the difference
    			// Currently it's O(n^2)

    			List<Integer> prevBins = getList(reduce, version);
    			List<Integer> currentBins = getList(reduce, currentVersion);
    //			System.out.println("rohanvbins: getReducerBins " + Arrays.toString(currentBins.toArray()));
    			
    			returnBins = new ArrayList<Integer>();

    			Iterator<Integer> currentIterator = currentBins.iterator();

    			while (currentIterator.hasNext()) {
    				Integer element = currentIterator.next();
    				if (!prevBins.contains(element)) {
    					returnBins.add(element);
    				}
    			}

    			// // Find the common elements
    			// while (prevIterator.hasNext()){
    			// element = prevIterator.next();
    			// if (currentBins.contains(element)){
    			// diffBins.add(element);
    			// }
    			// }
    			//      
    			// //Add all other elements except common.
    			// Iterator diffIterator - diffBins.iterator();
    			// while (diffIterator.hasNext()){
    			// element = diffIterator.next();
    			// if (!currentBins.contains(element)){
    			// returnBins.add(element);
    			// }
    			// }
    ...
    public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf,
      ...
      virtualMappingAll = new VirtualMappingAll(conf);
    }
    ...
    public boolean fetchOutputs() throws IOException {
      ...
      long lastOutputTime = 0;
        ...
        while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
          ...
          // TODO: Examine the following change
          synchronized (virtualMappingAll) {
            if (virtualMappingAll.needUpdate()) {
              // Wait for all the current fetching thread finishing
              List<MapOutputLocation> tempScheduledCopies = new ArrayList<MapOutputLocation>(
                  scheduledCopies.size());
              synchronized (scheduledCopies) {
                tempScheduledCopies.addAll(scheduledCopies);
                scheduledCopies.clear();
              }
              synchronized (copiers) {
                for (MapOutputCopier copier : copiers) {
                  copier.waitForFinish();
                }
              }
              // Re-fetch for all the completed tasks
              // Set the latest mapping table as current
              virtualMappingAll.switchToLatest();
              // Reload the scheduleCopies
              synchronized (scheduledCopies) {
                scheduledCopies.addAll(tempScheduledCopies);
                
              }
              // Clear all current copiedMapOutputs
              copiedMapOutputs.clear();
              // add back to mapLocations.
              synchronized (completeFetches) {
                Iterator<MapOutputLocation> locItr = completeFetches.iterator();
                while (locItr.hasNext()) {
                	
                    
                  MapOutputLocation loc = locItr.next();
                  System.out.println("rohancompletefetches " + loc.getTaskId().toString() );
                  List<MapOutputLocation> locList = mapLocations.get(loc
                      .getHost());
                  // May have problem if the Map output location mapping is cleared.
                  // Ignore it currently
                  if (locList != null) {
                    locList.add(0, loc);
                 //   LOG.info(reduceTask.getTaskID() + ": " + "refetch " + loc.getTaskId());
                  }
                }
                completeFetches.clear();
              }
            }
          }
            ...
            while (hostsItr.hasNext()) {
              ...
              synchronized (knownOutputsByLoc) {
                ...
                while (locItr.hasNext()) {
                  ...
                  System.out.println("rohanscheduledCopies: " + loc.getTaskId().toString() + " _numschedule_ " + numScheduled);
          ...
          while (numInFlight > 0 && mergeThrowable == null) {
            ...
            if (cr.getSuccess()) {  // a successful copy
            	/* Rohan Change on Sep 15 to change the numCopied to correct value */
           //   numCopied++;
              numCopied = copiedMapOutputs.size();
  //            System.out.println("ReduceTask: numCopied= " + numCopied);
              lastProgressTime = System.currentTimeMillis();
              reduceShuffleBytes.increment(cr.getSize());
          System.out.println("Tracking-Shuffle: " + System.currentTimeMillis() / 1000  + " " 
              + numCopied + " " + numInFlight + " " + numScheduled + " " + reduceShuffleBytes.getCounter() );
    ...
    private RawKeyValueIterator createKVIterator(
      ...
      System.out.println("rohanmerge: tmpDir is " + tmpDir.toString());
      ...
      if (mapOutputsFilesInMemory.size() > 0) {
        ...
        if (numMemDiskSegments > 0 &&
          ...
          System.out.println("rohanmerge: outputPath is " + outputPath.toString());
      ...
      //File mydir = new File("/export/gandhir/hadoop/mapred/local/taskTracker/gandhir/jobcache");
      ...
      public void run() {
        try {
          ...
          while(!exitLocalFSMerge){
        	  System.out.println("SortExternal-Wait: " + System.currentTimeMillis() / 1000);
            synchronized (mapOutputFilesOnDisk) {
              while (!exitLocalFSMerge &&
            ...
            System.out.println("SortExternal-Free: " + System.currentTimeMillis() / 1000);
            ...
            Path outputPath = mapFiles.get(0).suffix(".merged");
            Writer writer = 
            ...
            System.out.println("SortExternal-Done: " + System.currentTimeMillis() / 1000
            		+ " " + ((System.currentTimeMillis() / 1000) - reduceTask.startTime));
    ...
    private class InMemFSMergeThread extends Thread {
      ...
      private void doInMemMerge() throws IOException{
        ...
        TaskID mapId = mapOutputsFilesInMemory.get(0).mapId;
        int vbinId = mapOutputsFilesInMemory.get(0).vbinID;
        ...
        // mergeOutputSize);
        /* Rohan change on Sept 26: To change the file name to avoid conflicts */
        Path outputPath = new Path("map_" + mapId.toString() + "_vbin_" + Integer.toBinaryString(vbinId));
 //       System.out.println("Merge: " + outputPath.toString());
    ...
    private class GetMapEventsThread extends Thread {
      ...
      private int getMapCompletionEvents() throws IOException {
        ...
        // TODO: get update of the mapping table
        VirtualMappingUpdate vmu = umbilical.getVirtualMappings(
            reduceTask.getJobID(), virtualMappingAll.getMaxVersion() + 1);
        synchronized (virtualMappingAll) {
          virtualMappingAll.add(vmu.getVirtualMappers());
        }
        ...
        for (TaskCompletionEvent event : events) {
         ...
         //       System.out.println("rohanmapLocations: added " + loc.get(0).getTaskId().toString());
               }
