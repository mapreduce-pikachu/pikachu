...
class MapTask extends Task {
	...
	if (numReduceTasks > 0) {
	    MapOutputBuffer myVar = new MapOutputBuffer(umbilical, job, reporter);
		
	    String[] virtualBin = conf.getStrings("maxVirtualBins");
	    myVar.setPartition(Integer.parseInt(virtualBin[0]));
	    collector = (MapOutputCollector) myVar;
	...
	private int virtualPartitions;
    
	private int[] eachReducerShare; // = new double[conf.getNumReduceTasks()];
	private int[] reducerMap;
	...
	@SuppressWarnings("unchecked")
	    ...
	    //        virtualPartitions = conf.getNumVirtualReduceTasks();
    	
	    int binLimit = 0;
	    int currentBin = 0;
              
	    String inputArrayTmp[] = conf.getStrings("maxVirtualBins"); //new double[numPartitions];
	    int inputArray[] = new int[ conf.getNumReduceTasks() ];

	    /*
	    for (int i = 0; i<conf.getNumReduceTasks();i++){
        	inputArray[i] = Integer.parseInt(inputArrayTmp[i]);
		currentBin += inputArray[i];
	    }
	    */
	    virtualPartitions = Integer.parseInt(inputArrayTmp[0]);
	    numPartitions = virtualPartitions;
	    //	collector.setPartition(currentBin);
	    //collector.collect(key, value, partition)

	    currentBin = 0;
	    System.out.println("Old Dir enabled");
	    if (numPartitions > 0) {
		partitioner = (Partitioner<K,V>)
		    ReflectionUtils.newInstance(conf.getPartitionerClass(), conf);
        
		//        setReducerArraySize (conf, virtualPartitions);
		//        copyReducerShare (inputArray, conf);
		/*
		  for (int i = 0; i < numPartitions; i++){
		  binLimit = (int)(eachReducerShare[i]);//(int)(eachReducerShare[i] * virtualPartitions);
		  for (int j = currentBin; j < currentBin + binLimit; j++){
		  reducerMap[j] = i;
		  System.out.println("Initializing VB " + j + " goes to " + i);
		  }
		  currentBin += binLimit;      
		  }
		...
		*/
		...
		/*
		  for (int i = 0; i < numPartitions; i++){
		  binLimit = (int)(eachReducerShare[i] * virtualPartitions);
		  for (int j = currentBin; j < currentBin + binLimit; j++){
      		  reducerMap[j] = i;
		  System.out.println("Initializing VB " + j + " goes to " + i);
		  }
	...
	@Override
		...
		int hashBin = partitioner.getPartition(key, value, virtualPartitions);
		//      	collector.collect(key, value, reducerMap[hashBin]);
		// System.out.println("The hash bin is " + hashBin);
		collector.collect(key, value, hashBin);
	...
	private void setReducerArraySize (JobConf conf, int reducerSize){
	    eachReducerShare = new int[conf.getNumReduceTasks()];
	    reducerMap = new int[reducerSize];
	}
          
	private void copyReducerShare (int[] inputArray, JobConf conf){
	    System.arraycopy(inputArray, 0, eachReducerShare, 0, conf.getNumReduceTasks());
	}
    }
	...
	@SuppressWarnings("unchecked")
	    ...
	    throws IOException, ClassNotFoundException, InterruptedException {
	    this.reporter = reporter;
	    System.out.println("New Dir enabled");
	    Statistics matchedStats = null;
	...
	@SuppressWarnings("unchecked")
	    ...
	    MapOutputBuffer myVar = new MapOutputBuffer<K,V>(umbilical, job, reporter);
	    String[] virtualBin = conf.getStrings("maxVirtualBins");
	    myVar.setPartition(Integer.parseInt(virtualBin[0]));
	    collector = (MapOutputCollector)myVar;
	    //	    collector = (MapOutputCollector)<K,V>myVar;
	    ...
	    virtualPartitions = Integer.parseInt(inputArrayTmp[0]);
	    partitions = virtualPartitions;

	    /*
	    setReducerArraySize (virtualPartitions);
	    copyReducerShare (inputArray);
	    */
	...
	@Override
	    ...
	    /*    	
			if (hashBin < (virtualPartitions - 1)){
	  	  	if (reducerMap[hashBin] != reducerMap[hashBin + 1]){
			collector.collect(key, value, reducerMap[hashBin + 1]);
			//System.out.println("Key added to upper " + hashBin + " " + (hashBin + 1));
	  	  	}
			}
			...
			if (hashBin > 0){
	  	  	if (reducerMap[hashBin] != reducerMap[hashBin - 1]){
			collector.collect(key, value, reducerMap[hashBin - 1]);
			//			System.out.println("Key added to lower " + hashBin + " " + (hashBin - 1));
	  	  	}
			}
	    */
	...
	private void setReducerArraySize (int reducerSize){
	    eachReducerShare = new int[conf.getNumReduceTasks()];
	    reducerMap = new int[reducerSize];
        }
        
	private void copyReducerShare (int[] inputArray){
	    System.arraycopy(inputArray, 0, eachReducerShare, 0, conf.getNumReduceTasks());
        }
	...
	public void setPartition(int numPartitions){
	    partitions = numPartitions;
	    System.out.println("Setting the partition to " + partitions);
	}
	...
	public synchronized void collect(K key, V value, int partition
		...
		try {
		    ...
		    //System.out.println("The number of partition is " + partitions);
	...
	private void sortAndSpill() throws IOException, ClassNotFoundException,
		...
		for (int i = 0; i < partitions; ++i) {
			...
			// close the writer
			// System.out.println("Bin " + i + " has " + spindex);
			writer.close();
