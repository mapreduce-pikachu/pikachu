...
class TaskInProgress {
  ...
  private boolean beforeReducer = false;
  
  //Rohan change Feb 24
  // For the virtual bins
  private String taskMachine; // This is used only for the reducers.
  boolean cpuMonitorStarted = false; 
...
//Rohan change Feb 24
  // For the virtual bins
  public void setTaskOffset(String taskMachine){
	  this.taskMachine = taskMachine;
	    
  }
  
  public String getTaskOffset(){
  ...
  public void setBeforeReducer(boolean state){
	  beforeReducer = state;
  }
  
  public boolean getBeforeReducer( ){
	  return(beforeReducer);
  }
  ...
  boolean hasSpeculativeTask(long currentTime, double averageProgress) {
	...
	try {
	    BufferedWriter out = new BufferedWriter(new FileWriter("/export/gandhir/mystraggler.txt", true));
	    out.write("Straggler found\n");
	    out.close();
	} catch (IOException e) {
